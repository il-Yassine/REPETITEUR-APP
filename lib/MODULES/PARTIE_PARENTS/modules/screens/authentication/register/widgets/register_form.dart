...
class ParentRegisterFormScreen extends StatefulWidget {
  const ParentRegisterFormScreen({super.key, this.selectedRole});

  final Role? selectedRole;

  @override
  State<ParentRegisterFormScreen> createState() =>
      _ParentRegisterFormScreenState();
}

class _ParentRegisterFormScreenState extends State<ParentRegisterFormScreen> {
  ...
  Future<void> _fetchParentsRoleId() async {
    try {
      String roleId = await fetchParentsRoleId();
      setState(() {
        selectedRole = roleId;
      });
    } catch (error) {
              ...
              children: [
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: Consumer<ParentRegisteringProvider>(
                        builder: (context, parent_register, child) {
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        if (parent_register.resMessage != '') {
                          showMessage(
                              message: parent_register.resMessage,
                              context: context);
                          parent_register.clear();
                        }
                      });
                      bool status = parent_register.isLoading;
                      return AppFilledButton(
                        ...
                        onPressed: () {
                          if (_formKey.currentState!.validate()) {
                            ...
                            parent_register.registerParents(
                        ...
                        context: context,
                        buttonStatus: parent_register.isLoading,
                      );
                    }),
                  ),
                ),
